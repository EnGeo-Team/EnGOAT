...
def kMC(tunnel_system, N_kMC_runs, N_kMC_steps, T, tunnel_idx):                                             #Performs kMC simulation of a given tunnel system at temperature T
    ...
    a_vec = np.array([data.grid[0][1], data.grid[0][2], data.grid[0][3]])*data.grid[0][0]
    b_vec = np.array([data.grid[1][1], data.grid[1][2], data.grid[1][3]])*data.grid[1][0]
    c_vec = np.array([data.grid[2][1], data.grid[2][2], data.grid[2][3]])*data.grid[2][0]
    a_vec = a_vec/np.linalg.norm(a_vec)
    b_vec = b_vec/np.linalg.norm(b_vec)
    c_vec = c_vec/np.linalg.norm(c_vec)
    cell_xyz = np.vstack([a_vec*data.grid_size[0]*data.grid[0][0], b_vec*data.grid_size[1]*data.grid[1][0], c_vec*data.grid_size[2]*data.grid[2][0]])                                             
    ...
    for run in range(N_kMC_runs):
        ...
        for direction in range(3):                                                                           
            ...
            cond1 = msd[:, direction + 1] > (data.grid_size[direction]*data.grid[direction][0])**2           #Find start and end indices of the linear range of msd (between 1 and 4 unit cells)               
            cond2 = msd[:, direction + 1] > 4 * (data.grid_size[direction]*data.grid[direction][0])**2
    ...
    return D_average, D_xyz_average, D_3D_average