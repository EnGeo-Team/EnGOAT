...
class AutoCorrect(IntEnum):
    OFF = 0
    ADJUST = 1
    STOP_AT_BREAKTHROUGH = 2

AUTOCORRECT = {
    "off": AutoCorrect.OFF,
    "on": AutoCorrect.ADJUST,
    "stop_at_breakthrough": AutoCorrect.STOP_AT_BREAKTHROUGH,
}
...
# TuTraSt parameters
E_unit = ENERGY_UNITS[params["TuTraSt"]["E_unit"]]
E_step = params["TuTraSt"]["E_step"]
E_cutoff = params["TuTraSt"]["E_cutoff"]
autocorrect = AUTOCORRECT[params["TuTraSt"]["Dynamic_E_step"]]

# kMC parameters
run_kMC = params["kmc"]["run"]
T_list = params["kmc"]["temperatures"]
N_Temp = len(T_list)
N_kMC_steps = params["kmc"]["steps"]
N_kMC_runs = params["kmc"]["runs"]
particle_mass = params["kmc"]["particle_mass"]
...
# Functions defined to make the restart logic possible
def reset_analysis():
    """Reset all analysis data before restarting with a new E_step."""
...
def TuTraSt_analysis(autocorrect, E_step):
    ...
    logger.info(f"{'Number of levels:':<25} {N_levels:>10}")
    logger.info(f"{'E step:':<25} {E_step:>10}")
    logger.info(f"{'E cutoff:':<25} {E_cutoff:>10}")
    logger.info("-" * 80)
    logger.info(f"{'Level':>6} {'Energy (kJ/mol)':>17} {'Volume (pts)':>15} {'Breakthrough':>15} {'dt(s)':>10} {'Total(s)':>10}")
    logger.info("-" * 80)

    UC_histogram = {}

    """Explore the potential energy grid one energy level at a time.
    First, grow all existing clusters at the current level: identify all cluster combinings and merges,
    transition states, and breakthroughs (tunnels).
    Then, find new clusters at the current level.
    """

    breakthrough_level = np.full(3, level_max, dtype=int)               # Stores the levels at which the breakthrough in a, b, and c direction first occurred
    breakthrough = "/"                                                  # Unit cell directions in which the breakthrough occurs, just for display
    first_breakthrough_level = None

    for level in range(level_min, level_max):                           # Loop over all levels except the highest one
        E_volume[level] = np.sum(data.level_matrix <= level)            # Volume (number of grid points) at this energy level
        UC_histogram[(level-1) * E_step] = np.sum(data.level_matrix == level) * abs(np.linalg.det(data.grid["vectors"]))
        ...
        had_breakthrough = breakthrough != "/"                          # Flag indicating whether a breakthrough had already occurred

        if np.any(data.Tunnel.total_breakthrough_dimension):
            breakthrough = str(data.Tunnel.total_breakthrough_dimension)

            for dim in range(3):
                if data.Tunnel.total_breakthrough_dimension[dim] and level < breakthrough_level[dim]:
                    breakthrough_level[dim] = level
        ...
        # Record the first breakthrough level
        if not had_breakthrough and breakthrough != "/":
            first_breakthrough_level = level

            if autocorrect == AutoCorrect.STOP_AT_BREAKTHROUGH:
                break

    restart = False

    # Adjust E_step and restart if the first breakthrough occurred outside the desired range
    if autocorrect == AutoCorrect.ADJUST and first_breakthrough_level is not None:
        if not (10 <= first_breakthrough_level <= 20):
            restart = True

            if first_breakthrough_level < 10:
                E_step /= (20 // first_breakthrough_level)
            else:
                E_step *= (first_breakthrough_level // 10)

    return restart, E_step, N_levels, level_max, level_min, breakthrough_level, UC_histogram


while True:                                                             # E_step adjustment routine
    E_step_old = E_step

    restart, E_step, N_levels, level_max, level_min, breakthrough_level, UC_histogram = TuTraSt_analysis(autocorrect, E_step)
...
unit_cell = {"A_tot": A_tot, "A_acc": A_acc, "V_tot": V_tot, "V_acc": V_acc, "V_UC": V_UC, "V_B_tot": V_B_tot, "histogram": UC_histogram}
...
for TS in data.Transition_state.Transition_state_list:
    ...
    elif TS.clusters[0] == -1:                                                                                                        #Treat the same-cluster transition states (make it easier for later)
        basins = (TS.clusters[1], TS.clusters[1])
    else:
        basins = TS.clusters
...
logger.info(f"{'Total volume:':<25} {V_tot:10.4f} A^3 {V_tot/V_UC:10.4f} %")
logger.info(f"{'Accessible volume:':<25} {V_acc:10.4f} A^3 {V_acc/V_UC:10.4f} %")
logger.info(f"{'Total surface area:':<25} {A_tot:10.4f} A^2 {A_tot/V_UC:10.4f} A^2/A^3")
logger.info(f"{'Accessible surface area:':<25} {A_acc:10.4f} A^2 {A_acc/V_tot:10.4f} A^2/A^3")
logger.info("")
...
for tunnel_system in data.Tunnel_system.Tunnel_system_list:
    ...
    logger.info("-" * 80)
    logger.info(f"{'Volume:':<25} {tunnel_system.V:10.4f} A^3 {tunnel_system.V/V_UC:10.4f} %")
    logger.info(f"{'Surface area:':<25} {tunnel_system.A:10.4f} A^2 {tunnel_system.A/V_UC:10.4f} A^2/A^3")
    ...
    for cluster in tunnel_system.cluster_family:
        ...
        for process in tunnel_system.process_list:
            if process.start_cluster==cluster:
                ...
                process_list.append({"end_basin": str(process.end_cluster), "TS": str(process.transition_state), "cross_vector": list(process.process_cross_vector)})
    ...
    tunnel_systems[tunnel_system.ID]["graph_network"] = graph_dict
...
logger.info("-" * 80)
logger.info("Volume")
logger.info(f"  {'Total:':<15} {V_tot:10.4f} A^3 {V_tot/V_UC:10.4f} %")
logger.info(f"  {'Accessible:':<15} {V_acc:10.4f} A^3 {V_acc/V_UC:10.4f} %")
logger.info("")
logger.info("Surface area")
logger.info(f"  {'Total:':<15} {A_tot:10.4f} A^2 {A_tot/V_UC:10.4f} A^2/A^3")
logger.info(f"  {'Accessible:':<15} {A_acc:10.4f} A^2 {A_acc/V_UC:10.4f} A^2/A^3")
logger.info("-" * 80)
logger.info("Breakthrough energies")
logger.info("-" * 80)
if breakthrough_level[0]!=level_max and Ea_a<E_cutoff:
    logger.info(f"{'Direction a:    level'} {breakthrough_level[0]:>3}    E = {breakthrough_level[0]*E_step:10.3f} kJ/mol     dE = {Ea_a:10.3f} kJ/mol")
else:
    logger.info(f"{'Direction a:    level'} {'/':>3}    E = {'/':10} kJ/mol     dE = {'/':10} kJ/mol")
if breakthrough_level[1]!=level_max and Ea_b<E_cutoff: 
    logger.info(f"{'Direction b:    level'} {breakthrough_level[1]:>3}    E = {breakthrough_level[1]*E_step:10.3f} kJ/mol     dE = {Ea_b:10.3f} kJ/mol")
else:
    logger.info(f"{'Direction b:    level'} {'/':>3}    E = {'/':10} kJ/mol     dE = {'/':10} kJ/mol")
if breakthrough_level[2]!=level_max and Ea_c<E_cutoff: 
    logger.info(f"{'Direction c:    level'} {breakthrough_level[2]:>3}    E = {breakthrough_level[2]*E_step:10.3f} kJ/mol     dE = {Ea_c:10.3f} kJ/mol")
else:
    logger.info(f"{'Direction c:    level'} {'/':>3}    E = {'/':10} kJ/mol     dE = {'/':10} kJ/mol")
logger.info("-" * 80)
    
if len(data.Tunnel_system.Tunnel_system_list)!=0 and run_kMC==1:
    logger.info("Diffusion coefficients")
    logger.info("-" * 80)
    logger.info(f"{'T (K)':>8}  {'D_a (cm^2/s) +- s':>22}  {'D_b (cm^2/s) +- s':>22}  {'D_c (cm^2/s) +- s':>21}")
    logger.info("-" * 80)
