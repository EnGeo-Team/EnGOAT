...
import get_process_cross_vector
...
def _compute_cross_vector(start_point, end_point, C1_C2_TS, temp_cluster_matrix):

    return get_process_cross_vector.get_process_cross_vector(
        start_point,
        end_point,
        C1_C2_TS,
        temp_cluster_matrix
...
def get_process_list(cluster_family):
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for transition_state in transition_state_list:
            ...
            if -1 in transition_state.clusters:
                continue
            ...
            if not (cluster1.boundary or cluster2.boundary):
                cross_vectors[transition_state.ID]=np.zeros(3,dtype=int)
                continue

            start_point=np.asfortranarray(cluster1.center,dtype=np.int64)
            end_point=np.asfortranarray(cluster2.center,dtype=np.int64)

            C1_C2_TS=np.asfortranarray(
                (cluster1.ID,cluster2.ID,transition_state.ID),
                dtype=np.int64
            )
            ...
            for transition_state_2 in transition_state_list:
                if C1_C2_TS[0] in transition_state_2.clusters and \
                   C1_C2_TS[1] in transition_state_2.clusters and \
                   C1_C2_TS[2] != transition_state_2.ID:

                    mask=data.TS_matrix.IDs==transition_state_2.ID
                    temp_cluster_matrix[mask]=0

            future=executor.submit(
                _compute_cross_vector,
                start_point,
                end_point,
                C1_C2_TS,
                temp_cluster_matrix
            ...
            futures[future]=transition_state.ID

        for future,ts_id in futures.items():
            cross_vectors[ts_id]=future.result()
    ...
    for transition_state in transition_state_list:
        ...
        if -1 in transition_state.clusters:
            ...
            processforward=data.Process(
                ...
                start_point,
                dE,
                process_cross_vector
            )

            processbackward=data.Process(
                cluster1_ID,
                cluster1_ID,
                transition_state.ID,
                start_point,
            ...
            process_cross_vector=cross_vectors[transition_state.ID]
            ...
            if tuple(process_cross_vector)==(100,100,100):
    ...
    return process_list, TS_list

    
def Organize_tunnel_systems(level_max):
    ...
    for cluster_family in data.Cluster.Cluster_families:
        ...
        if len(family_tunnel_list)==0:                                                                  #Skip cluster families for which no tunnel was identified
            ...
            process_list, transition_state_list = get_process_list(cluster_family)

            height = level_max
            for cluster_ID in cluster_family:
                if data.Cluster.Cluster_list[cluster_ID - 1].E_min < height:
                    height = data.Cluster.Cluster_list[cluster_ID - 1].E_min

            data.Isolated_group.Isolated_group_list.append(
                data.Isolated_group(
                    ISO_ID,
                    cluster_family,
                    height,
