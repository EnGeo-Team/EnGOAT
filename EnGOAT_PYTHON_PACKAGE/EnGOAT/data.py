...
def create_matrices(E_step, E_cutoff, E_unit, cube_file):
    ...
    global atoms, grid                                                  #1) number of atoms and origin from the .cube file, 2) grid information from the .cube file stored as a dictionary; "shape" contains np.array of N grid points in a gicen direction, "vectors" contains 3x3 np.array containing a [0], b[1], and c[2] voxel vector information
    ...
    atoms, grid, Energy_matrix = Read_cube_data.Read_cube_data(E_unit, cube_file) 
    ...
    Cluster_matrix = ID_matrix(grid["shape"])
    TS_matrix = ID_matrix(grid["shape"])
    Tunnel_matrix = ID_matrix(grid["shape"])
    cross_matrix = CROSS_matrix(grid["shape"])
...
class Cluster:                                                          #Each cluster formed is an object with the following attributes:
    def __init__(self, ID, cluster_points, E_min, center, point_energies):                      
        ...
        self.V = None                                                   #Volume of the basin
        self.A = None                                                   #Area of the basin
        self.V_rel = None
        self.A_rel = None
        self.histogram = None                                           #Energy volume histogram (volume of accessible points per energy level, saved as a dictionary {E_level:V})
        self.V_B = None                                                 #Boltzmann weighted volume of a cluster at a given temperature saved as a dictionary {T:V_B}. Used for constructing a graph network output.
...
class Isolated_group:
    def __init__(self, ID, cluster_family, height, transition_state_list, process_list):
        self.ID=ID                                                      #Identity number
        self.cluster_family=cluster_family                              #Cluster family forming a tunnel
        self.height=height                                              #Energy value of the lowest energy point within the tunnel system
        self.transition_state_list=transition_state_list                #list of all transition states (objects, see above) between clusters of the tunnel system
        ...
        self.histogram = None                                           #Energy volume histogram (volume of accessible points per energy level, saved as a dictionary {E_level:V})
        self.V_B = None                                                 #Boltzmann weighted volume of a cluster at a given temperature saved as a dictionary {T:V_B}. Used for constructing a graph network output.
    Isolated_group_list=[]                                              #List of all tunnel systems identified in the structure
