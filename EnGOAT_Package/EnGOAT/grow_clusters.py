        ...
        for current_cluster in data.Cluster.Cluster_list:                                           
            ...
            if (Filling_matrix[current_cluster.ID-1]==0) and (current_cluster.active):                                                  #checks if the cluster has not yet been filled or removed
                ...
                for current_point_index in boundary_points:         
                    ...
                    neighbour_points=find_neighbours(level, current_point, data.cross_matrix.i, data.cross_matrix.j, data.cross_matrix.k)
                    boundary=0    
...
def find_neighbours(level, point,
                    cross_matrix_i,
                    cross_matrix_j,
                    cross_matrix_k):
    """
    Find the 6 periodic-boundary-condition neighbours of a point.
    ...
    point : array-like, shape (9,)
        Current point. Python coordinates are assumed to be zero-based:
        point[0:3] = x, y, z.

    cross_matrix_i, cross_matrix_j, cross_matrix_k : np.ndarray
        3D arrays containing crossing information.

    Returns
    -------
    neighbours : np.ndarray, shape (6, 9), dtype=np.int64
        Six neighbouring points.
    """

    # Grid dimensions
    nx, ny, nz = cross_matrix_i.shape

    # Python zero-based coordinates
    i, j, k = point[:3].astype(np.int64)

    # Current crossing information
    ...
    neighbours = np.empty((6, 9), dtype=np.int64)

    # +x neighbour
    neighbours[0] = [
        ip, j, k,
        cross_i + cross_x_positive,
        cross_j,
        cross_k,
        1,
        level,
